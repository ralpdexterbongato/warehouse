        ...
        <div class="MCT-title">
          @if (($MCTMast[0]->IssuedbySignature!=null)&&($MCTMast[0]->ReceivedbySignature!=null))
            <form class="print-buttonMCT"action="{{route('print-mct')}}" method="post">
              {{ csrf_field() }}
              <input type="text" autocomplete="off" name="MIRSNo" value="{{$MCTMast[0]->MIRSNo}}" style="display:none">
          ...
          @endif
          @if (($MCTMast[0]->IssuedbySignature!=null)&&($MCTMast[0]->ReceivedbySignature!=null))
          @if ((($MRTcheck[0]==null)&&(Auth::user()->Role==4))||(($MRTcheck[0]==null)&&(Auth::user()->Role==4)))
          ...
          @elseif($MRTcheck[0]!=null)
            <div class="View-MRT-btn">
              <div class="mrt-done">
              <a href="{{route('mrt-viewer',[$MRTcheck])}}"><button type="submit"><i class="fa fa-eye eyesicon"></i> View MRT</button></a>
              </div>
            </div>
          @else
