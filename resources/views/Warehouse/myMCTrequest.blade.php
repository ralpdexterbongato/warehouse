        ...
        <tr>
          @foreach ($myrequestMCT as $myMCT)
            <tr>
              ...
              <td><i class="fa fa-eye" onclick="$('#fullmct{{$myMCT->MCTNo}}').submit()"></i></td>
              <form id="fullmct{{$myMCT->MCTNo}}" action="{{route('previewMCT')}}" method="GET">
                <input type="text" name="MIRSNo" value="{{$myMCT->MIRSNo}}" style="display:none">
              </form>
            </tr>
          @endforeach
        </tr>
