  ...
  <div class="setting-accounts-table">
    ...
    <table>
      ...
      <tr v-for="manager in Managers">
        <td><h2><p>{{manager.Fname}}</p></h2></td>
        <td>{{manager.Lname}}</td>
        <td>{{manager.Username}}</td>
        <td><h1><img :src="'/storage/signatures/'+manager.Signature" alt="signature"></h1></td>
        <td class="userstatus">
          <i v-if="manager.IsActive!=null" class="fa fa-circle active"></i>
  ...
  export default {
     data () {
      ...
      methods: {
        ...
        {
          var vm=this;
          axios.get(`/get-all-managers?page=`+page).then(function(response)
        {
