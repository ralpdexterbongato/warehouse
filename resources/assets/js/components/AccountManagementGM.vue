<template lang="html">
  <div class="setting-accounts-table">
    ...
    <table>
      ...
      <tr v-for="gm in GeneralManagers">
        <td><h2><p>{{gm.Fname}}</p></h2></td>
        <td>{{gm.Lname}}</td>
        <td>{{gm.Username}}</td>
        <td><h1><img :src="'/storage/signatures/'+gm.Signature" alt="signature"></h1></td>
        <td class="userstatus">
          <i v-if="gm.IsActive!=null" class="fa fa-circle active"></i>
  ...
  export default {
     data () {
      ...
      methods: {
        ...
        {
          var vm=this;
          axios.get(`/get-general-managers?page=`+page).then(function(response)
        {
